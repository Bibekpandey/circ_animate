export async function run(ctx, fps=2000, steps=2000) {
    const scale = 1;
    ...
    for(let i=0; i<steps ;i++) {
        newPos = sequence.next().value * scale;
        diameter = newPos - currPos;
        let anticlockwise = i % 2 !== 0;
        if (diameter < 0) anticlockwise  = !anticlockwise;
        //let startAngle, endAngle;
        const startAngle = diameter > 0 ? Math.PI : 0;
        const endAngle = diameter > 0 ? 2* Math.PI : Math.PI;
        const X = currPos + diameter / 2;
        const rad = Math.abs(diameter/2);
        currPos = newPos;
        //ctx.arc(X, Y, rad, startAngle, endAngle);
        await animateArc(ctx, X, Y, rad, startAngle, endAngle, anticlockwise, fps);
    }
}
